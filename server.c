...
char* globaldeck();     // any number of cards from 1 full deck of 52 cards
char* structdeck();     // any number of cards from 1 full deck of 52 cards

void * context;

// global deck
char g_cards[104] = { 'A',3,'2',3,'3',3,'4',3,'5',3,'6',3,'7',3,'8',3,'9',3,'0',3,'J',3,'Q',3,'K',3     // (char)3 == ♥
...
int main()
{
    ...
    while(1)
    {
        ...
        {
            if(strstr(buffer, "Damian>>>deck>>>draw>>>") != 0)
            {
                for(int i = strlen("Damian>>>deck>>>draw>>>"); buffer[i] != '\0'; i++)
                {
                    temp[j++] = buffer[i];
                }

                strcat(send, globaldeck(atoi(temp)));
                j = 0;
                memset(temp, 0, strlen(temp));
            }
            else if(strstr(buffer, "Damian>>>deck>>>shuffle>>>") != 0)
            {
                sprintf(g_cards, "%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c"
                                 ...
                                 ,'A',6,'2',6,'3',6,'4',6,'5',6,'6',6,'7',6,'8',6,'9',6,'0',6,'J',6,'Q',6,'K',6);   // (char)6 == ♠
                g_amount = 52;
                strcat(send, "Deck has been shuffled");
            }
            else if(strstr(buffer, ">>>draw>>>") != 0)
            {
                for(int i = strlen("Damian>>>deck>>>"); buffer[i] != '>'; i++)
                {
                    temp[j++] = buffer[i];
                }
                ...
                {
                    if(players[i].sessionID != '\0' && strstr(players[i].sessionID, temp) != 0)
                    {
                        for(int k = strlen("Damian>>>deck>>>") + strlen(players[i].sessionID) + strlen(">>>draw>>>"); buffer[k] != '\0'; k++)
                        {
                            temp[j++] = buffer[k];
                        }
                    }
                ...
                {
                    if(players[ii].sessionID == '\0')
                    {
                        players[ii].sessionID = (char*)malloc(sizeof(temp));
                        strcpy(players[ii].sessionID, temp);
                        sprintf(players[ii].cards, "%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c"
                                                    "%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c"
                                                     ,'A',3,'2',3,'3',3,'4',3,'5',3,'6',3,'7',3,'8',3,'9',3,'0',3,'J',3,'Q',3,'K',3     // (char)3 == ♥
                                                     ,'A',4,'2',4,'3',4,'4',4,'5',4,'6',4,'7',4,'8',4,'9',4,'0',4,'J',4,'Q',4,'K',4     // (char)4 == ♦
                                                     ,'A',5,'2',5,'3',5,'4',5,'5',5,'6',5,'7',5,'8',5,'9',5,'0',5,'J',5,'Q',5,'K',5     // (char)5 == ♣
                                                     ,'A',6,'2',6,'3',6,'4',6,'5',6,'6',6,'7',6,'8',6,'9',6,'0',6,'J',6,'Q',6,'K',6);   // (char)6 == ♠
                        players[ii].amount = 52;
                        ii = 30;
                    }
                }

                j = 0;
                strcat(send, structdeck(atoi(temp)));
                memset(temp, 0, strlen(temp));
            }
            else
            {
...
char* globaldeck(int amount)
{
    ...
    for(int i = 0; i < amount; i++)
    {
        ...
        if(g_cards[pos] != '\n')
        {
            returncards[returnpos++] = g_cards[pos];
            returncards[returnpos++] = g_cards[pos + 1];
            returncards[returnpos++] = '\n';
            g_cards[pos] = '\n';
            g_cards[pos + 1] = '\n';
        }
...
char* structdeck(int index, int amount)
{
    int pos = 0;
    int returnpos = 0;
    char* returncards = (char*) malloc(3 * amount * sizeof(int));
    ...
    if(amount > 52)
        return "There are only 52 cards in a deck, enter a number less than or equal to 52";
    if(players[index].amount < amount)
        return "Not enough cards in deck";
    ...
    {
        pos = 2 * (rand() % 52);
        if(players[index].cards[pos] != '\n')
        {
            returncards[returnpos++] = players[index].cards[pos];
            returncards[returnpos++] = players[index].cards[pos + 1];
            returncards[returnpos++] = '\n';
            players[index].cards[pos] = '\n';
            players[index].cards[pos + 1] = '\n';
        }
        else
        {
            i--;
